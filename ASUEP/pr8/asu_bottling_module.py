...
class ASUBottlingSystem:
    ...
    def update_sensor_value(self, sensor_id: int, new_value: float):
        ...
        try:
            query = (
                "UPDATE industrial_sensors SET current_value = %s WHERE sensor_id = %s"
            )
    ...
    def generate_production_report(
        self, technologist_id: int, report_type: str, days_back: int = 1
    ) -> Dict:
        """
        ...
        try:
            ...
            efficiency = round(
                ((total_bottles - reject_count) / total_bottles) * 100, 2
            )
...
if __name__ == "__main__":
    ...
    if user:
        ...
        if recipes:
            print(
                f"Текущая рецептура: {recipes[0]['regulation_name']} (Версия: {recipes[0]['version']})"
            )
        ...
        if tech_id_row:
            report = asu.generate_production_report(
                tech_id_row[0], "DAILY", days_back=1
            )
            if report:
                print(
